...
defun:
          DEF FUNCNAME '(' a_list ')' '{' l_def stmt_list '}'  { 
		$2->type = func;
		$$ = opr(FDEFA,4, id(slookup($2->sym)), $8, $4, $7); }
        | DEF FUNCNAME '(' ')' '{' l_def stmt_list '}'         {
		$2->type = func;
		$$ = opr(FDEF, 3, id(slookup($2->sym)), $7, $6); 
		debug($6);
	}
        | INTER FUNCNAME '(' ')' '{' l_def stmt_list '}'         {
		$2->type = isr;
		$$ = opr(INTER, 3, id(slookup($2->sym)), $7, $6); 
		debug($6);
	}
...
astmt:
        ...
        | '*' VARIABLE '=' expr           { $$ = opr(MWT, 2, id(slookup($2->sym)), $4); }
...
expr:
        ...
        | ',' FUNCNAME %prec UMINUS { $$ = opr(EA, 1, id(slookup($2->sym))); }
        | '*' VARIABLE %prec UMINUS { $$ = opr(MRD, 1, id(slookup($2->sym))); }
